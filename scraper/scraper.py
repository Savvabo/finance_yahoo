...
def transform_historical_data(company_data: dict) -> List[HistoryRow]:
    ...
    adj_close_col = map(lambda num: round(num, 2), indicators['adjclose'][0]['adjclose'])
    volume_col = map(lambda num: round(num, 2), quote_data['volume'])

    history_table = zip(date_col, open_col, high_col, low_col, close_col, adj_close_col, volume_col)
    # get company name from json
    company_name = company_data['chart']['result'][0]['meta']['symbol']
    history_table_rows = list(map(lambda tuple_row: HistoryRow(*tuple_row, company_name=company_name), history_table))
